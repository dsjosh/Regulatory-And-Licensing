...
export default function Home() {
    ...
    async function checkSession() {
      ...
      if (!data.loggedIn) {
        ...
        if (data.role === 'officer') {
          ...
          void fetchOperatorInProgress();
          void fetchOperatorCompleted();
        }
      }
    }
  ...
  const fetchAllOperators = async () => {
    const res = await fetch('/api/officer/get-operators'); 
    const data = await res.json();
  ...
  const startInspection = async () => {
    ...
    const res = await fetch('/api/officer/inspections/new', {
      method: 'POST',
  ...
  const fetchInProgress = async () => {
    const res = await fetch(`/api/officer/inspections/in-progress`);
    const data = await res.json();
  ...
  const fetchCompleted = async () => {
    const res = await fetch(`/api/officer/inspections/completed`);
    const data = await res.json();
  ...
  // Operator API Calls
  const fetchOperatorInProgress = async () => {
    const res = await fetch(`/api/operator/inspections/in-progress`);
    const data = await res.json();
    setInProgressList(data.inspections);
    setInProgressCount(data.count);
  };

  const fetchOperatorCompleted = async () => {
    const res = await fetch(`/api/operator/inspections/completed`);
    const data = await res.json();
    setCompletedList(data.inspections);
    setCompletedCount(data.count);
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main className="max-w-4xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
        <div className="text-center bg-white rounded-3xl p-8 sm:p-12 shadow-xl shadow-slate-200/50 border border-slate-100 mb-8">
          <h1 className="text-4xl sm:text-5xl leading-normal font-extrabold tracking-tight bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent mb-4">
            Homepage
          </h1>
            ...
            <div className="mt-8 text-left border-t pt-8">
              ...
              {activeTab === 2 && (
                <div className="space-y-4">
                  ...
                  <input type="text" placeholder="Filter by operator email" className="border p-2 rounded w-full" value={inProgressSearch} onChange={(e) => setInProgressSearch(e.target.value)} />
          ...
          {user?.role === 'operator' && (
             <div className="mt-8 text-left border-t pt-8">
               <div className="flex gap-4 mb-6">
                 <button className={`px-4 py-2 rounded ${activeTab === 1 ? 'bg-indigo-600 text-white' : 'bg-slate-100'}`} onClick={() => setActiveTab(1)}>In-Progress</button>
                 <button className={`px-4 py-2 rounded ${activeTab === 2 ? 'bg-indigo-600 text-white' : 'bg-slate-100'}`} onClick={() => setActiveTab(2)}>Completed</button>
               </div>
  ...
  {completedList.map((insp, idx) => (
    <div key={idx} className="p-3 border rounded bg-slate-50">
      ...
      <div className="text-sm text-slate-500">
        Started: {insp.start_date}
      </div>
      <div className="text-sm text-slate-500">
        Completed: {insp.end_date}
      </div>
    </div>
  ))}
</div>
